...
ReturnCode rfalWakeUpModeStart( void *config )
{
    ...
    if( config == NULL)
    {
        gRFAL.wum.cfg.period  = RFAL_WUM_PERIDOD_500MS;
        ...
        gRFAL.wum.cfg.indAmp.reference = RFAL_WUM_REFRENCE_AUTO;
    ...
    reg |= ((gRFAL.wum.cfg.period < RFAL_WUM_PERIDOD_100MS) ? ST25R3911_REG_WUP_TIMER_CONTROL_wur : 0x00);
    ...
    if( gRFAL.wum.cfg.indAmp.enabled )
    {
        ...
        {
            if( gRFAL.wum.cfg.indAmp.reference == RFAL_WUM_REFRENCE_AUTO )
            {
    ...
    if( gRFAL.wum.cfg.indPha.enabled )
    {
        ...
        {
            if( gRFAL.wum.cfg.indPha.reference == RFAL_WUM_REFRENCE_AUTO )
            {
